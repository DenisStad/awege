...
class XcodeProject(PBXDict):
    ...
    def _printNewXCodeFormat(self, out, root, deep, enters=True, sort=False):
        if isinstance(root, UserDict):
            ...
            for key in sorted(root.items()):  # keep the same order as Apple.
                if enters:
                    out.write('\t' + deep)
                key = key[0]
            ...
            if len(root) > 0 and re.match(regex, root).group(0) == root:
                ...
                out.write('"' + XcodeProject.addslashes(str(root)) + '"')

            if root in self.uuids:
                out.write(" /* " + str(self.uuids[root]) + " */")
